...
function App(){
  ...
  async function enableLocation(){
    ...
    if(!window.isSecureContext){
      setGeoMsg('A localização só funciona em conexão segura (HTTPS).'); setGeoHelp(true); return;
    }
    if(!navigator.geolocation){
      setGeoMsg('Este dispositivo ou navegador não oferece geolocalização.'); setGeoHelp(true); return;
    }
    try{
      if(navigator.permissions?.query){
        const permission=await navigator.permissions.query({name:'geolocation' as PermissionName});
        if(permission.state==='denied'){
          setGeoMsg('O acesso à localização está bloqueado no navegador.'); setGeoHelp(true); return;
        }
      }
    }catch{}
    setGeoLoading(true);
